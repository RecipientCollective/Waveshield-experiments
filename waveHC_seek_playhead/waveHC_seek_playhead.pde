...
void playcomplete(FatReader &file) {
  ...
  while (wave.isplaying) {
     ...
     if ( ((potval - lastpotval) > HYSTERESIS) || ((lastpotval - potval) > HYSTERESIS)) {
         ...
         putstring("remainbyteinchunk = ");
         Serial.println(wave.remainingBytesInChunk, DEC);
        
         //newchunk = wave.remainingBytesInChunk;
         
         newchunk = potval;
         putstring("newchunk potval-> ");
         Serial.println(newchunk);
         newchunk *= totalsize;
         putstring("newchunk * totalsize -> ");
         Serial.println(newchunk);
         newchunk /= 1024;
         putstring("newchunk/1024 -> ");
         Serial.println(newchunk);
...
//        }        
        ...
        wave.seek(newchunk);
        putstring(" newposition-> ");
        Serial.println(newchunk, DEC);
        lastpotval = potval;
     }
     delay(100);
   }
